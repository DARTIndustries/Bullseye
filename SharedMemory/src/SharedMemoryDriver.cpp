...
volatile location_shm_t *SharedMemoryDriver::open_location_shm() {
...
/** Returns a pointer to a shared memory map. This pointer can freely be written to without locks */
void SharedMemoryDriver::detach_location_shm(location_shm_t * shm) {
    shmdt(shm); 
}
...
*/
void SharedMemoryDriver::destroy_location_shm() {
    key_t key = ftok(POSITION_SHM_FILE, POSITION_SHM_KEY_ID);
    int shmid = shmget(key, 1024, 0666|IPC_CREAT);
